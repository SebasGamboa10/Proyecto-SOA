...
ipcMain.on('python-script', async (event, arg) => {

  try {
    const result = await runPythonScript(arg);
...
ipcMain.handle('run-python', async (event, scriptPath) => {
  try {
    const result = await runPythonScript(scriptPath);
    return result;
  } catch (error) {
    console.error('Error running Python script:', error);
    throw error; // Re-throw for handling in the renderer process
  }
});
